...
\textbf{Hướng B --- lập kế hoạch với mô hình chính xác.} Thay vì \emph{học} một sở thích mục tiêu, bộ lập kế hoạch \emph{tính} ra nó. Vì luật của trò chơi đã biết, ta mô phỏng tương lai gần một cách chính xác và chọn các đợt phóng làm kết quả dự báo có lợi cho ta nhất. Đây là lập kế hoạch dựa trên mô hình theo nghĩa chân trời lùi / điều khiển dự báo theo mô hình (MPC)~\cite{mpc1989, suttonbarto}: mô hình được \emph{cho trước}, không phải học.
...
\item \textbf{Dựng hoặc làm mới mô hình chuyển động.} \texttt{PlanetMovement} dự báo vị trí mọi hành tinh trong chân trời $H$, theo dõi hạm đội đang bay bằng một sổ cái (ledger), và hỗ trợ dự báo tiến lực lượng đồn trú.
...
\paragraph{Danh sách rút gọn} Nguồn là các hành tinh ta sở hữu có ít nhất \texttt{min\_ships\_to\_launch} tàu, xếp theo số tàu. Mục tiêu gồm một danh sách \emph{tấn công} (hành tinh địch hoặc trung lập theo khoảng cách) và một danh sách \emph{phòng thủ} các \textit{mục tiêu lật} (\textit{flip})---hành tinh của ta được dự báo sẽ mất trong chân trời, xếp theo độ khẩn cấp $=\text{sản lượng}\cdot(\text{số lượt còn lại}) + \text{số tàu}$.

\paragraph{Định cỡ và khả năng tới được} Với mỗi (nguồn, đích), cỡ đợt phóng là \texttt{safe\_drain}: số tàu nhiều nhất nguồn có thể gửi mà vẫn tự giữ được qua chân trời. Một kiểm tra khả năng tới được dạng giải tích (có tính đến trôi quỹ đạo, mặt trời che tầm nhìn, và độ lệch bề mặt) cùng một bộ giải \texttt{intercept\_angle} (lặp điểm bất động) cho ra góc ngắm và ETA tới mục tiêu di chuyển.
...
\subsection{Cấu hình}
Một \texttt{ProducerLiteConfig} cố định giữ các thiết lập. Cấu hình 2p dùng chân trời $=18$, tối đa 6 đợt sóng mỗi lượt, ROI $=1{,}5$, và focus-fire bật (tối đa 4 nguồn đánh). Cấu hình 4p rút chân trời xuống 13 và giảm số nguồn, số mục tiêu phòng thủ, và số nguồn đánh, vì một trận bốn người có nhiều phân nhánh hơn và tầm look-ahead hữu ích ngắn hơn.
