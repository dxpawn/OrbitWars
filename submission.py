...
class Hellburner:
    ...
    # NEW: Phase-aware value weights (NovaHeuristic-style)
    ...
    def plan_midgame(self, deadline):
        ...
        ATTENTION: Use attention-based target prioritization from friend's model."""
        moves: FleetOrders = []
        ...
        for _ in range(self.SEARCH_MAX_ACTIONS):
            ...
            # Use attention-based target selection if enabled
            if ATTN_ENABLED:
                # Get attention-prioritized targets from all planets
                attn_targets = []
                for src in self.owned_planets:
                    if src.ships == 0:
                        continue
                    max_travel = self.MAX_DISTANCE
                    targets = _attn_nearest_targets(src, self, 16, max_travel, set())
                    for target, _ in targets:
                        if target not in attn_targets:
                            attn_targets.append(target)
                
                # Evaluate attention-prioritized targets first
                for target in attn_targets:
                    if time.perf_counter() >= deadline:
                        break
                ...
                # Fall back to original sorting by ships
                for target in sorted(self.planets, key=lambda p: p.ships, reverse=True):
                    if time.perf_counter() >= deadline:
                        break
                    action = self._action_for_target(target)
    ...
    def main(self, obs: dict[str, Any]) -> list[Any]:
        ...
        # NEW: Update phase detection (NovaHeuristic-style)
        opening_turn = self.OPENING_TURN_2P if self.n_sides <= 2 else self.OPENING_TURN_4P
