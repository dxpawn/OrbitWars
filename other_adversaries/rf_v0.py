...
# FFA anti-kingmaking / anti-MAD tuning:
# Attack the leader, harvest weak/overextended players late, and avoid long
# value-negative wars with non-leaders while neutral growth is still available.
...
def simulate_planet_timeline(planet, arrivals, player, horizon):
    ...
    # One dense pass creates the queryable base timeline. This keeps the
    # expensive work front-loaded instead of re-simulating every candidate.
    ...
    if planet.owner == player:
        # Fast path: without hostile arrivals, the planet never needs a reserve.
        # This removes the binary-search survival loop for the common case.
...
def target_value(target, arrival_turns, mission, world, modes, policy):
    ...
    if target.id in world.comet_ids:
        ...
        # Strict Comet Viability Filter
        # cost is approx target.ships + 1. recoup_time = cost / production.
    ...
    if mission == "snipe":
        ...
        if modes["is_ffa"]:
            value *= 1.5  # The Vulture Protocol: heavily favor enemy crashes in FFA
    ...
    # FFA-specific: build safe production and avoid mutually destructive wars.
    if modes["is_ffa"]:
...
def preferred_send(
    ...
    # Dynamic Margin Decay
    # Scale margin by remaining steps / TOTAL_STEPS
...
def plan_moves(world, deadline=None):
    ...
    # The Spoofing Protocol (Ghost Fleets)
    # Every 18 turns, send 1 ship to the strongest enemy to paralyze their AI.
