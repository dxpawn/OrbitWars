...
MAX_SPEED = 6.0
DECOY_THRESHOLD = 8
MAX_ATTACK_DISTANCE = 45.0  # Giới hạn cự ly chiến đấu cục bộ cực kỳ mạnh mẽ
...
def ray_hits_target(sx: float, sy: float, angle: float, tx: float, ty: float, tr: float) -> bool:
    fvx, fvy = math.cos(angle), math.sin(angle)
    dx, dy = tx - sx, ty - sy
    proj = dx * fvx + dy * fvy
    if proj <= 0:
        return False
...
# ============================================================
# Thuật toán BFS tìm đường tiếp vận tối ưu (BFS Reinforcement)
# ============================================================

def get_reinforcement_targets(my, enemy, planets):
    adj = {p['id']: [] for p in my}
    for src in my:
        for dst in my:
            if src['id'] == dst['id']:
                continue
            d = math.hypot(src['x'] - dst['x'], src['y'] - dst['y'])
            # Giới hạn cự ly an toàn của chặng tiếp viện
            if d <= 42.0:
    ...
    for p in my:
        is_front = False
        for e in enemy:
            if math.hypot(p['x'] - e['x'], p['y'] - e['y']) <= 42.0:
                is_front = True
                break
        if is_front:
            frontline.add(p['id'])

    # BFS loang tìm đường đi ngắn nhất về hướng tiền tuyến
    hops_to_front = {pid: 0 for pid in frontline}
    parent_path = {pid: None for pid in frontline}
    queue = list(frontline)
    head = 0
    while head < len(queue):
        node_id = queue[head]
        head += 1
        for neighbor_id in adj[node_id]:
            if neighbor_id not in hops_to_front:
                hops_to_front[neighbor_id] = hops_to_front[node_id] + 1
                parent_path[neighbor_id] = node_id
...
def agent(obs):
    ...
    for src in my:
        ...
        if phase == 'expand':
            ...
            for t in neutrals:
                ...
                # Chặn đứng các hành trình mở rộng quá xa không hiệu quả
                if d > MAX_ATTACK_DISTANCE:
                    continue
            ...
            if best_target:
                ...
                if not is_comet_and_leaving and not path_crosses_sun(src['x'], src['y'], ix, iy, margin=1.5):
                    ...
                    if ray_hits_target(src['x'], src['y'], angle, ix, iy, best_target['radius']):
                ...
                if decoy_tgt and src['ships'] > 25:
                    ...
                    if send >= 5:
                        ...
                        if not is_comet_and_leaving and not path_crosses_sun(src['x'], src['y'], ix, iy, margin=1.5):
                            angle = safe_angle(src['x'], src['y'], ix, iy)
                            if ray_hits_target(src['x'], src['y'], angle, ix, iy, decoy_tgt['radius']):
                                moves.append([src['id'], angle, send])
        ...
        if phase == 'counter_attack':
            ...
            if best_enemy:
                ...
                if not is_comet_and_leaving and not path_crosses_sun(src['x'], src['y'], ix, iy, margin=1.5):
                    ...
                    if src['ships'] > send + 3:
                        angle = safe_angle(src['x'], src['y'], ix, iy)
                        if ray_hits_target(src['x'], src['y'], angle, ix, iy, best_enemy['radius']):
                            moves.append([src['id'], angle, send])
        ...
        angle = safe_angle(src['x'], src['y'], ix, iy)
        if ray_hits_target(src['x'], src['y'], angle, ix, iy, tgt['radius']):
            moves.append([src['id'], angle, send])
    ...
    if phase == 'expand':
        for src in my:
            ...
            if best_tgt:
                ...
                if not is_comet_and_leaving and not path_crosses_sun(src['x'], src['y'], ix, iy, margin=1.5):
                    angle = safe_angle(src['x'], src['y'], ix, iy)
                    if ray_hits_target(src['x'], src['y'], angle, ix, iy, best_tgt['radius']):
    ...
    # === PHA ĐIỀU PHỐI TIẾP VẬN BFS (BFS Reinforcement Belt) ===
    # Đẩy quân dồn dập từ hậu phương lên tiền tuyến mỗi 4 lượt (tăng tempo đáng kể)
    if step % 4 == 0 and len(my) > 1 and enemy:
        parent_path = get_reinforcement_targets(my, enemy, planets)
        for src in my:
            if src['id'] in in_flight_from:
                continue
            tgt_id = parent_path.get(src['id'])
            if tgt_id is not None:
                tgt = planets[tgt_id]
                # Giữ lại thủ vệ tối thiểu là 15 tàu để phòng thủ hậu phương an toàn
                if src['ships'] >= 15 + 10:
                    send = int(src['ships'] - 15)
                    r = math.hypot(tgt['x'] - SUN_X, tgt['y'] - SUN_Y)
                    is_orbiting = (r + tgt['radius']) < 48.0
                    ix, iy, tt = solve_intercept(src['x'], src['y'], tgt['x'], tgt['y'], tgt['id'], is_orbiting, omega, send, comets, comet_ids)
                    if not path_crosses_sun(src['x'], src['y'], ix, iy, margin=1.5):
                        angle = safe_angle(src['x'], src['y'], ix, iy)
                        if ray_hits_target(src['x'], src['y'], angle, ix, iy, tgt['radius']):
                            moves.append([src['id'], angle, send])
                            src['ships'] -= send
                            in_flight_from.add(src['id'])

    # Spoofing Protocol (Ghost Fleets)
            ...
            if strongest_planets:
                ...
                if best_src['ships'] > 30:
                    ...
                    if not path_crosses_sun(best_src['x'], best_src['y'], ix, iy, margin=1.5):
                        ...
                        if ray_hits_target(best_src['x'], best_src['y'], angle, ix, iy, target['radius']):
