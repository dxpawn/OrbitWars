"""NeRL ("Not even RL") -- a model-based, model-predictive planner for Orbit Wars.

NeRL carries no neural network and no learned weights. It builds an exact forward
model of the game, projects every planet's future ownership and garrison over a
horizon, and selects launches by their marginal effect on a competitive ship-flow
objective: greedy multi-wave selection, capture-floor ship economy, coordinated
multi-source strikes (focus-fire) and a defensive regroup pass. This is model-based
planning with a *given* model (receding-horizon / MPC), i.e. the "planning" side of
the course rather than learning.

torch is used only as a fast vectorised-math backend so a full per-turn simulation
fits inside the one-second budget. Entry point: agent(obs).
"""
