"""Heuristic v3 — heuristic_v2 with 4-player-aware target selection.

Inherits all of v2 (which is bug-fixed hellburner). The only addition is
leader-aware target value:

  In 2-player play, target value remains exactly `target.production` (same
  as v2 / hellburner).

  In >=3-player play, the score for an enemy planet owned by the strongest
  enemy (largest production + ships total) is multiplied by 1.4. This
  pushes attack effort toward denying the leader rather than scrapping
  with the weakest opponent — which matters because only #1 wins on
  Kaggle's reward function (4p ties at top all win, but #2 doesn't).

This is conservative: 2p behaviour is byte-identical to v2, and 4p only
shifts target priority — not the combat sim, defence pass, or
reinforcement layer. Worst case it ties v2.
"""
...
class Hellburner:
    ...
    # v3 addition: in 4p, planets owned by the strongest enemy get value * this.
    LEADER_TARGET_BOOST: float = 1.4

    def __init__(self):
        self.player: int = 0
        self.scene_step: int = 0
        self.angular_velocity: float = 0.0
        # v3: leader detection (only meaningful when n_players >= 3)
        self.strongest_enemy: int | None = None
    ...
    def evaluate_move_orders(self) -> MoveOrders:
        ...
        for target in sorted(self.planets, key=lambda p: p.ships, reverse=True):
            ...
            # not owned
                ...
                if (target.owner == -1):
                    value = value - 1
                # v3: in 4p, boost value of leader's planets so we deny rather
                # than scrap with the weakest enemy. No-op in 2p (strongest_enemy is None).
                elif (self.strongest_enemy is not None
                      and target.owner == self.strongest_enemy):
                    value = value * self.LEADER_TARGET_BOOST
    ...
    def main(self, obs: dict[str, Any]) -> list[Any]:
        ...
        # v3: detect n_players + strongest enemy (by production + ships).
        # Skip neutrals (owner == -1) and ourselves.
        per_owner_strength: dict[int, float] = defaultdict(float)
        seen_owners: set[int] = set()
        for p in self.planets:
            if p.owner == -1:
                continue
            seen_owners.add(p.owner)
            if p.owner != self.player:
                # weight production heavily (it's the long-term value)
                per_owner_strength[p.owner] += p.production * 8.0 + p.ships
        for f in self.fleets:
            if f.owner == -1 or f.owner == self.player:
                continue
            seen_owners.add(f.owner)
            per_owner_strength[f.owner] += f.ships
        # n_players: how many distinct owners we've seen so far (incl. us).
        self.n_players = max(2, len(seen_owners | {self.player}))
        if self.n_players >= 3 and per_owner_strength:
            self.strongest_enemy = max(per_owner_strength,
                                       key=per_owner_strength.get)
        else:
