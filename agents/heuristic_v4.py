"""Heuristic v4 — heuristic_v2 + a single multiplayer threat re-calibration.

Inherits both of v2's bug-fixes vs the original hellburner (see below).

The ONE behavioural change vs v2:
  - In evaluate_frontline_strategy, before committing a neighbor's ships to an
    attack, v2 checks whether that neighbor would survive a worst-case scenario
    where EVERY connected enemy attacks it simultaneously at 50% strength. With
    one opponent (2p) that is a reasonable worst case. With three opponents (4p
    FFA) it massively over-counts threat — the opponents are busy fighting each
    OTHER, not gang-focusing our single planet. The result is excessive
    passivity in 4p (measured: v2 issues empty moves on ~63% of 4p turns vs
    ~21% in 2p). v4 scales the per-attacker pressure fraction by 2/n_sides:
        2 sides (2p)        -> 0.50  (IDENTICAL to v2: provably a no-op in 2p)
        3 sides             -> 0.333
        4 sides (full 4p)   -> 0.25
    n_sides counts active sides (us + distinct enemy owners with a planet or
    fleet), so as opponents are eliminated the model reverts toward v2's
    focused worst-case. This loosens 4p over-conservatism while leaving the 2p
    behaviour — which drives the bulk of the Kaggle score — byte-for-byte
    unchanged.
...
class Hellburner:
    ...
    def __init__(self):
        ...
        # turn in main(); used to discount worst-case focus-fire in multiplayer.
        self.n_sides: int = 2
    ...
    def evaluate_frontline_strategy(self, target: HPlanet) -> tuple[FleetOrders, list[Intercept], bool]:
        ...
        for neighbor, _ in possible_origins:
            ...
            if not_doomed:
                ...
                half_pressure = 0
                # v4: discount per-attacker focus-fire by the number of active
                # sides. 2 sides -> 0.5 (identical to v2). More sides -> less,
                # because mutually-hostile opponents won't all gang up at once.
                threat_frac = 0.5 * (2.0 / max(2, self.n_sides))
                for attacker, _ in self.inbound_edges.get(neighbor, []):
                    ...
                    if not math.isfinite(atk_travel):
                        continue
                    half_ships = max(1, int(attacker.ships * threat_frac))
    ...
    def main(self, obs: dict[str, Any]) -> list[Any]:
        ...
        # v4: count active sides (us + distinct enemy owners with a planet or
        # fleet). Drives the multiplayer threat discount in evaluate_frontline_strategy.
